...
def acc(y_true, y_pred):
    ...
    return K.mean(K.equal(y_true > 0.5, K.round(y_pred) > 0.5), axis=-1)


class MyGan():
    def __init__(self, realRoot, fakeRoot, chRoot):
        self.trainRato = 0.9
        ...
        self.logPath = os.path.join(chRoot, 'log.csv')
        with open(self.logPath, 'w') as f:
            f.write('dRealLoss,dRealAcc,dFakeLoss,dFakeAcc,valLoss,valAcc\n')
    ...
    def getGenerator(self):
        ...
        x = Conv2D(1, 7, activation='relu', padding='same')(x)
    ...
    def getDiscriminator(self):
        ...
        discriminator_optimizer = keras.optimizers.Adam(lr=1e-4, decay=1e-8)
        discriminator.compile(optimizer=discriminator_optimizer, loss='binary_crossentropy', metrics=[acc])
    ...
    def trainGan(self):
        ...
        gan_input = keras.Input(shape=(8, 8, 1))
        gan_output = self.dModel(self.gModel(gan_input))
        self.gan = keras.models.Model(gan_input, gan_output)

        # gan_optimizer = keras.optimizers.RMSprop(lr=4e-4, clipvalue=1.0, decay=1e-8)
        gan_optimizer = keras.optimizers.Adam(lr=1e-4, decay=1e-8)
        self.gan.compile(optimizer=gan_optimizer, loss='binary_crossentropy', metrics=[acc])
        ...
        for e in range(self.epoch):
            ...
            for _ in range(0, len(self.realPaths), self.half_batch_size):
                ...
                for _p in self.realPaths[_:_ + self.half_batch_size]:
                    with open(_p, 'rb') as f:
                        realx.append(pkl.load(f)['suv'][1, :, :, np.newaxis])
                        # realy.append(1 - 0.1 * np.random.random())  # 真实数据用1表示,并在标签中加入一些随机
                        realy.append(1 - 0.1 * np.random.random())
                ...
                for _p in self.fakeTrainPaths[_:_ + self.half_batch_size]:
                    with open(_p, 'rb') as f:
                        ...
                        fakex.append(x[:, :, np.newaxis])
                        # fakey.append(0.1 * np.random.random())  # 伪造数据用0表示,并在标签中加入一些随机
                        fakey.append(0)
                self.dModel.trainable = True
                self.gModel.trainable = False
                for xxx in range(self.trainDTimes):  # 判别器多训练几次
                    dBatchFakeLoss, dBatchFakeAcc = self.gan.train_on_batch(np.array(fakex), np.array(fakey))
                    dBatchRealLoss, dBatchRealAcc = self.dModel.train_on_batch(np.array(realx),
                                                                               np.reshape(realy, (-1, 1)))
                ...
                if e > self.gModelBeginEpochThres:
                    self.dModel.trainable = False
                    self.gModel.trainable = True
                    ganBatchLoss, ganBatchAcc = self.gan.train_on_batch(np.array(fakex), np.ones((np.shape(fakex)[0],)))
                    batchTime = time.time() - beginTime
                    processBar = '%04d/%04d' % (e, self.epoch) + '  [' + '=' * int(20 * trainedRate) + ' ' * int(
                        20 * (1 - trainedRate)) + '] ' + '%.2f' % trainedRate + ' leftTime:' + str(batchTime * len(
                        self.realPaths) // self.half_batch_size * (1 - trainedRate))
            ...
            with open(self.logPath, 'a') as f:
                f.write('%d,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f\n' % (e,dEpochRealLoss, dEpochRealAcc, dEpochFakeLoss,
    ...
    def dRealGenerator(self, paths, batchSize):
        ...
        for _ in range(0, len(paths), batchSize):
            ...
            for _p in paths[_:_ + batchSize]:
                with open(_p, 'rb') as f:
                    xs.append(pkl.load(f)['suv'][1, :, :, np.newaxis])
                    ys.append(1)  # 真实数据用1表示,并在标签中加入一些随机
            yield np.array(xs), np.array(ys)

    def save_imgs(self, generator, imgPaths, e, op=r'E:\pyWorkspace\NewCAE\GAN\geneImg'):
        xs = []
        if e == -1:
            for i, _p in enumerate(imgPaths):
                with open(_p, 'rb') as f:
                    x = pkl.load(f)['suv'][1, :, :]
                    skio.imsave(os.path.join(op, 'e=%d_i=%d_p=%s.tif' % (e, i, os.path.basename(_p))), x)
        else:
            for i, _p in enumerate(imgPaths):
                with open(_p, 'rb') as f:
                    x = pkl.load(f)['suv'][1, :, :]
                    if x.shape != (8, 8):
                        x = resize(x, (8, 8), order=1, preserve_range=True, )
                    xs.append(x[:, :, np.newaxis])
            res = generator.predict(np.array(xs))
            for i in range(res.shape[0]):
                skio.imsave(os.path.join(op, 'e=%d_i=%d.tif' % (e, i)), res[i, :, :, 0])


if __name__ == '__main__':
    realRoot = r'E:\pyWorkspace\CAE\res\highSuvPatch\bigPatch'
    fakeRoot = r'E:\pyWorkspace\CAE\res\highSuvPatch\smallPatch'
    gan = MyGan(realRoot, fakeRoot, chRoot=r'E:\pyWorkspace\NewCAE\GAN\chp')
    gan.trainGan()
